...
function mobileSpeakerBlock(owner,parts,c,th){
  const kind=owner?.kind||'char',name=owner?.name||(kind==='pc'?(persona(th)?.name||state.profile.name||'사용자'):c?.name||'캐릭터'),key=owner?.key||mobileSpeakerKey(kind,c,th,name),color=mobileSpeakerColor(th,kind==='char'?'character':kind,key,kind==='pc'?'#65b9ff':'#e1b722'),style=` style="--mobile-speaker-color:${color}"`;
  return `<section class="mobile-speaker-block ${kind}" data-speaker-key="${esc(key)}"${style}><b class="mobile-inline-speaker">${esc(name)}</b><div class="mobile-speaker-bubble">${parts.map(part=>proseHtml(part.content,kind,color,part.type)).join('')}</div></section>`
}
function renderMobileSemanticSegments(segments,c,th){
  const groups=[];let current=null;
  for(const seg of segments){const owner=seg.owner||null,signature=owner?`${owner.kind}|${owner.key}`:'narration';if(!current||current.signature!==signature){current={signature,owner,parts:[]};groups.push(current)}current.parts.push(seg)}
  return groups.map(group=>group.owner?mobileSpeakerBlock(group.owner,group.parts,c,th):`<section class="mobile-narration-block">${group.parts.map(part=>proseHtml(part.content,'narration','',part.type)).join('')}</section>`).join('')
}
function forcedStoryHtml(message,c,th){
  const type=message?.speakerType||'';if(!['player','pc','character','extra'].includes(type))return'';
  const kind=type==='player'||type==='pc'?'pc':type==='character'?'char':'extra',name=kind==='pc'?(message.speakerName||persona(th)?.name||state.profile.name||'사용자'):kind==='extra'?(message.speakerName||state.extras.find(x=>x.id===message.speakerId)?.name||'등장인물'):(message.speakerName||c?.name||'캐릭터'),owner={kind,name,key:mobileSpeakerKey(kind,c,th,name,message.speakerId)};
  return mobileSpeakerBlock(owner,[{type:'narration',content:message.content}],c,th)
}
function mobileStoryHtml(text,role='assistant',c=null,th=null,message=null){
  const raw=String(text||'');if(role==='user'){const key=mobileSpeakerKey('pc',c,th),color=mobileSpeakerColor(th,'pc',key,'#65b9ff');return `<p class="mobile-user-line" data-speaker-key="${esc(key)}" style="--mobile-speaker-color:${color}">${esc(raw).replace(/\n/g,'<br>')}</p>`}
  const forced=forcedStoryHtml(message,c,th);if(forced)return forced;
  const rendered=[];for(const seg of taggedSegments(raw)){
    if(seg.type==='legacy'){rendered.push(renderMobileSemanticSegments(mobileLegacySegments(seg.content,c,th),c,th));continue}
    if(seg.type==='narration'){rendered.push(`<section class="mobile-narration-block">${proseHtml(seg.content,'narration')}</section>`);continue}
    const name=seg.type==='pc'?(persona(th)?.name||state.profile.name||'사용자'):seg.type==='extra'?(seg.name||'등장인물'):(c?.name||'캐릭터'),owner={kind:seg.type,name,key:mobileSpeakerKey(seg.type,c,th,name)};rendered.push(mobileSpeakerBlock(owner,[{type:'narration',content:seg.content}],c,th))
  }return rendered.join('')
}
export function plainMessageText(text){return cleanForContext(text)}
export function messageHtml(m,c){const th=thread(),p=persona(th),isIntro=isThreadIntroSeed(m,th?.introId),narration=m.speakerType==='narration',who=narration?'지문':m.role==='user'?(m.speakerName||p?.name||state.profile.name||'사용자'):(m.speakerName||c?.name||'캐릭터'),u=m.usage||{},meta=!isIntro&&m.role==='assistant'?[Number(u.total_tokens||0)?`${Number(u.total_tokens).toLocaleString()}t`:'',Number(m.estimatedCostKRW||0)>0?`약 ₩${Math.max(1,Math.round(Number(m.estimatedCostKRW))).toLocaleString()}`:'',Number(m.latencyMs||0)>0?`${(Number(m.latencyMs)/1000).toFixed(1)}s`:''].filter(Boolean).join(' · '):'',variants=Array.isArray(m.variants)?m.variants:[];return`<article class="msg ${m.role}${narration?' narration':''}${isIntro?' intro-seed':''}${m.bookmarked&&!isIntro?' bookmarked':''}" data-id="${m.id}">${m.role==='assistant'&&!isIntro?`<header><span class="mobile-chat-avatar">${c?.avatar?`<img src="${esc(c.avatar)}" alt="">`:esc((c?.name||'?').slice(0,1))}</span><b>${esc(who)}</b>${m.bookmarked?'<span class="message-bookmark-mark">★</span>':''}</header>`:''}<div class="msg-body">${mobileStoryHtml(m.content,m.role,c,th,m)}</div>${meta?`<div class="mobile-message-usage">${esc(meta)}${m.autoContinuationCount?` · 자동 이어쓰기 ${m.autoContinuationCount}회`:''}</div>`:''}${isIntro?'':`<footer><button data-act="copy">복사</button><button data-act="edit">수정</button><button data-act="delete">삭제</button><button data-act="bookmark">${m.bookmarked?'★ 해제':'☆ 북마크'}</button>${m.role==='assistant'&&!narration?`<button data-act="regen">재생성</button>${variants.length>1?`<button data-act="variants">답변 후보 ${Number(m.activeVariantIndex||0)+1}/${variants.length}</button>`:''}<button data-act="branch">분기</button><button data-act="speak">읽기</button>`:''}</footer>${m.role==='assistant'&&!narration?widgetsHtml(m,th):''}`}</article>`}
